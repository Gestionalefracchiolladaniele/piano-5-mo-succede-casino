...
function smoothLerp(cur: number, target: number, f: number) {
...
// Elegant blue/white palette
const COLOR_DARK = "#0F2440";
const COLOR_WHITE = "#FFFFFF";
const COLOR_MUTED_BG = "#F6F8FC";
const COLOR_BORDER = "#E6EAF2";
const COLOR_TEXT = "#0F2440";

const FEATURES = [
  {
    num: "01",
    title: "Pipeline Intelligente",
    desc: "Gestisci lead e opportunità con una vista chiara. Ogni deal al posto giusto, sempre.",
    from: "left" as const,
  },
  {
    num: "02",
    title: "Automazioni Potenti",
    desc: "Elimina il lavoro manuale. Flussi automatici per email, follow-up e notifiche.",
    from: "bottom" as const,
  },
  {
    num: "03",
    title: "Analisi in Tempo Reale",
    desc: "Dashboard con metriche che contano davvero per il tuo business.",
    from: "right" as const,
  },
  {
    num: "04",
    title: "Integrazioni Native",
    desc: "Connettiti con gli strumenti che già usi. Tutto sincronizzato.",
    from: "bottom" as const,
  },
];
...
export default function Index() {
  ...
  useEffect(() => {
    const animate = () => {
      ...
      const tY = lerp(VH * 0.1, VH * 0.82, p);
    ...
    >
      <style>{`
        ...
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
        ...
        /* BUTTONS */
        .btn-w{display:inline-block;padding:16px 44px;background:${COLOR_WHITE};color:${COLOR_DARK};font-family:'Instrument Mono',monospace;font-size:10px;letter-spacing:.16em;text-transform:uppercase;text-decoration:none;border:1.5px solid rgba(255,255,255,.75);transition:all .3s;}
        .btn-w:hover{background:transparent;color:${COLOR_WHITE};}
        .btn-ghost{display:inline-block;padding:16px 44px;background:transparent;color:${COLOR_WHITE};font-family:'Instrument Mono',monospace;font-size:10px;letter-spacing:.16em;text-transform:uppercase;text-decoration:none;border:1.5px solid rgba(255,255,255,.3);transition:all .3s;}
        .btn-ghost:hover{background:rgba(255,255,255,.07);border-color:rgba(255,255,255,.6);}
        .btn-crm{display:inline-block;padding:20px 64px;background:${COLOR_WHITE};color:${COLOR_DARK};font-family:'Instrument Mono',monospace;font-size:12px;letter-spacing:.18em;text-transform:uppercase;text-decoration:none;border:1.5px solid ${COLOR_WHITE};transition:all .3s;}
        .btn-crm:hover{background:transparent;color:${COLOR_WHITE};}

        /* FEATURE CARD */
        .fcard{border:1px solid ${COLOR_BORDER};padding:40px 32px 44px;background:${COLOR_WHITE};position:relative;overflow:hidden;transition:border-color .35s,box-shadow .35s,transform .35s;}
        .fcard::after{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:${COLOR_DARK};transform:scaleX(0);transform-origin:left;transition:transform .45s cubic-bezier(.22,1,.36,1);}
        .fcard:hover::after{transform:scaleX(1);}
        .fcard:hover{border-color:rgba(15,36,64,.35);transform:translateY(-5px);box-shadow:0 16px 48px rgba(15,36,64,.10);}
        .bnum{position:absolute;bottom:-12px;right:8px;font-family:'Cormorant Garamond',serif;font-size:110px;font-weight:600;color:rgba(15,36,64,.05);line-height:1;pointer-events:none;user-select:none;transition:color .35s;}
        .fcard:hover .bnum{color:rgba(15,36,64,.10);}
      ...
      <div
        ...
        style={{
          width:
            typeof window !== "undefined" && window.innerWidth < 768 ? 90 : 150,
          height:
            typeof window !== "undefined" && window.innerWidth < 768 ? 90 : 150,
          opacity: 0,
        }}
      ...
      <div ref={impactRef} className="impact">
        <div
          style={{
            ...
            inset: 0,
            background:
              "radial-gradient(circle,rgba(255,255,255,.20) 0%,transparent 70%)",
            borderRadius: "50%",
          }}
        />
        <div
      ...
      <nav
        style={{
          ...
          borderBottom: scrolled ? `1px solid ${COLOR_BORDER}` : "none",
      ...
      >
        <div
          style={{
            ...
            fontWeight: 600,
            letterSpacing: ".2em",
            color: scrolled ? COLOR_DARK : COLOR_WHITE,
            transition: "color .4s",
          }}
        >
          ARTEMISIA
        ...
        >
          <a
            href="#features"
            className="nav-lk"
            style={{ color: scrolled ? COLOR_DARK : COLOR_WHITE }}
          >
            Funzioni
          </a>
          <a
          ...
          <a
            ...
            style={{
              padding: "11px 28px",
              background: scrolled ? COLOR_DARK : "transparent",
              color: COLOR_WHITE,
              ...
              textDecoration: "none",
              border: `1.5px solid ${
                scrolled ? COLOR_DARK : "rgba(255,255,255,.55)"
              }`,
              transition: "all .3s",
            }}
          >
          ...
          {[0, 1, 2].map((i) => (
            <span
              ...
              style={{
                ...
                width: 22,
                height: 1.5,
                background: scrolled ? COLOR_DARK : COLOR_WHITE,
                transition: "all .3s",
                transform:
                  menuOpen && i === 0
      ...
      <section ref={heroRef} style={{ position: "relative", height: "100vh", overflow: "hidden" }}>
        ...
        {/* Dark overlay — now deep-blue overlay (elegant) */}
        <div
          style={{
            position: "absolute",
            inset: 0,
            background: `linear-gradient(to bottom,
            rgba(15,36,64,${0.55 - heroProgress * 0.1}) 0%,
            rgba(15,36,64,${0.18 - heroProgress * 0.05}) 40%,
            rgba(15,36,64,${0.75 - heroProgress * 0.1}) 100%)`,
          }}
        />
        ...
        >
          ...
          >
            <HeroLine show={showLine1} delay={0}>
              Il tuo
            </HeroLine>
            <HeroLine show={showLine2} delay={80} italic>
              business
            </HeroLine>
            <HeroLine show={showLine3} delay={160}>
              al centro.
            </HeroLine>
          </h1>
          ...
          <p
            style={{
              ...
              transition:
                "opacity .9s cubic-bezier(.22,1,.36,1) 0ms, transform .9s cubic-bezier(.22,1,.36,1) 0ms",
            }}
          >
        ...
        >
          <svg
            viewBox="0 0 68 68"
            style={{ width: 56, height: 56, animation: "rotateSlow 9s linear infinite" }}
          >
            <defs>
              <path
                id="cr"
                d="M 34,34 m -24,0 a 24,24 0 1,1 48,0 a 24,24 0 1,1 -48,0"
              />
            </defs>
            <text
              style={{
          ...
          >
            <div style={{ width: 1, height: 12, background: "rgba(255,255,255,.55)" }} />
            <div
              style={{
                borderLeft: "4px solid transparent",
                borderRight: "4px solid transparent",
                borderTop: "5px solid rgba(255,255,255,.55)",
              }}
            />
          </div>
        </div>
      ...
      {/* ══════════════════════════════════
          MARQUEE
      ══════════════════════════════════ */}
      <div style={{ overflow: "hidden", background: COLOR_DARK, padding: "26px 0" }}>
          ...
          {[...Array(4)].flatMap(() =>
            STATS_MARQUEE.map((s, i) => (
              <span key={`${s}-${i}`} style={{ display: "inline-flex", alignItems: "center", gap: 28, padding: "0 40px" }}>
                <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, fontWeight: 300, color: COLOR_WHITE }}>
                  {s}
                </span>
      ...
      <section id="features" style={{ padding: "120px 32px", maxWidth: 1200, margin: "0 auto" }}>
        <Reveal from="left">
          <span className="mono" style={{ fontSize: 10, opacity: 0.3, display: "block", marginBottom: 20 }}>
            — Funzionalità
          </span>
        </Reveal>
        <Reveal delay={80}>
            ...
            <Reveal key={f.num} from={f.from} delay={i * 80}>
              <div className="fcard">
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 28 }}>
                  <span className="mono" style={{ fontSize: 10, opacity: 0.3 }}>
                    {f.num}
                  </span>
                </div>
                ...
                <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 17, fontWeight: 300, lineHeight: 1.8, opacity: 0.62 }}>
                  {f.desc}
                </p>
              </div>
            </Reveal>
      ...
      {/* ══════════════════════════════════
          STATS
      ══════════════════════════════════ */}
      <section style={{ background: COLOR_MUTED_BG, padding: "100px 32px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <Reveal from="left">
            <span className="mono" style={{ fontSize: 10, opacity: 0.3, display: "block", marginBottom: 56 }}>
              — Numeri che parlano
            </span>
          </Reveal>
          <div className="stats-g" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32 }}>
            {[
              ...
              <Reveal key={s.val} from={i % 2 === 0 ? "left" : "right"} delay={i * 80}>
                <div style={{ borderTop: `1px solid rgba(15,36,64,.18)`, paddingTop: 28 }}>
                  <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(48px,5vw,72px)", fontWeight: 300, lineHeight: 1, marginBottom: 10 }}>
                    {s.val}
                  </div>
                  <div className="mono" style={{ fontSize: 9, opacity: 0.42 }}>
                    {s.label}
                  </div>
                </div>
              </Reveal>
      ...
      <section style={{ padding: "120px 32px", maxWidth: 860, margin: "0 auto", textAlign: "center" }}>
        <Reveal>
          <div style={{ fontSize: 64, opacity: 0.07, lineHeight: 1, marginBottom: 16, fontFamily: "Georgia,serif" }}>
            "
          </div>
          <blockquote
            style={{
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 14 }}>
            <div style={{ width: 38, height: 38, borderRadius: "50%", background: "rgba(15,36,64,.10)" }} />
            <div style={{ textAlign: "left" }}>
              <div className="mono" style={{ fontSize: 10, opacity: 0.65 }}>
                Marco Ferretti
              </div>
              <div className="mono" style={{ fontSize: 9, opacity: 0.32 }}>
                CEO, Nexlabs Milano
              </div>
            </div>
          </div>
      ...
      {/* ══════════════════════════════════
          CTA
      ══════════════════════════════════ */}
      <section id="crm" style={{ background: COLOR_DARK, padding: "160px 32px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center", position: "relative" }}>
          <Reveal from="left">
            <span className="mono" style={{ fontSize: 10, color: "rgba(255,255,255,.30)", display: "block", marginBottom: 28 }}>
              — Pronto a iniziare?
            </span>
          </Reveal>
          <Reveal delay={80}>
          ...
          <Reveal from="right" delay={140}>
            ...
            >
              Accedi alla dashboard, gestisci i tuoi clienti e fai crescere il tuo business da oggi e per sempre.
            </p>
      ...
      >
        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 17, fontWeight: 600, letterSpacing: ".2em", color: COLOR_WHITE }}>
          ARTEMISIA
        </div>
        <div className="mono" style={{ fontSize: 9, color: "rgba(255,255,255,.30)" }}>
          © {new Date().getFullYear()} Artemisia — Tutti i diritti riservati.
        </div>
      </footer>
    </div>
  );
